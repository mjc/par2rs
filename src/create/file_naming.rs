...
fn allocate_recovery_blocks(
    ...
    match scheme {
        ...
        RecoveryFileScheme::Limited => {
            ...
            let largest = if block_size == 0 {
                recovery_block_count.max(1)
            } else {
                largest_file_size.div_ceil(block_size).max(1) as u32
            };
...
mod tests {
    ...
    #[test]
    fn limited_scheme_falls_back_when_size_cap_cannot_fit_tail() {
        let plan = plan_recovery_files("test", 1, 2, 0, RecoveryFileScheme::Limited, 1, 1);
