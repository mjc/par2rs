...
impl CreateConfig {
    ...
    pub fn validate(&self) -> Result<(), super::CreateError> {
        ...
        if let Some(count) = self.source_block_count {
            if count.as_u32() == 0 || count.as_u32() > 32768 {
                return Err(CreateError::InvalidBlockCount(
                    "Block count must be between 1 and 32768".to_string(),
                ));
            }
        }

        if let Some(count) = self.recovery_block_count {
            if count > 32768 {
                return Err(CreateError::InvalidRecoveryBlockCount(
                    "Recovery block count must be at most 32768".to_string(),
                ));
            }
        }

        if let Some(count) = self.recovery_file_count {
            if !(1..=31).contains(&count) {
                return Err(CreateError::InvalidRecoveryFileCount(
                    "Recovery file count must be between 1 and 31".to_string(),
                ));
            }
            if self.recovery_block_count == Some(0) {
                return Err(CreateError::InvalidRecoveryFileCount(
                    "Recovery file count cannot be set when recovery block count is 0".to_string(),
                ));
            }
        }

        if self.first_recovery_block > 32768 {
            return Err(CreateError::InvalidFirstRecoveryBlock(
                "First recovery block must be at most 32768".to_string(),
            ));
        }
...
mod tests {
    ...
    #[test]
    fn validate_rejects_source_block_count_zero() {
        let c = CreateConfig {
            source_block_count: Some(SourceBlockCount::new(0)),
            ..valid_config()
        };
        assert!(c.validate().is_err());
    }
    ...
    #[test]
    fn validate_rejects_source_block_count_above_par2_limit() {
        let c = CreateConfig {
            source_block_count: Some(SourceBlockCount::new(32769)),
            ..valid_config()
        };
        assert!(c.validate().is_err());
    }
    ...
    #[test]
    fn validate_accepts_max_source_block_count() {
        let c = CreateConfig {
            source_block_count: Some(SourceBlockCount::new(32768)),
            ..valid_config()
        };
        assert!(c.validate().is_ok());
    }
    ...
    #[test]
    fn validate_rejects_recovery_block_count_above_par2_limit() {
        let c = CreateConfig {
            recovery_block_count: Some(32769),
            ..valid_config()
        };
        assert!(c.validate().is_err());
    }
    ...
    #[test]
    fn validate_accepts_max_recovery_block_count() {
        let c = CreateConfig {
            recovery_block_count: Some(32768),
            ..valid_config()
        };
        assert!(c.validate().is_ok());
    }
    ...
    #[test]
    fn validate_rejects_recovery_file_count_zero() {
        let c = CreateConfig {
            recovery_file_count: Some(0),
            ..valid_config()
        };
        assert!(c.validate().is_err());
    }
    ...
    #[test]
    fn validate_rejects_recovery_file_count_above_cli_limit() {
        let c = CreateConfig {
            recovery_file_count: Some(32),
            ..valid_config()
        };
        assert!(c.validate().is_err());
    }
    ...
    #[test]
    fn validate_rejects_recovery_file_count_with_zero_recovery_blocks() {
        let c = CreateConfig {
            recovery_block_count: Some(0),
            recovery_file_count: Some(1),
            ..valid_config()
        };
        assert!(c.validate().is_err());
    }
    ...
    #[test]
    fn validate_accepts_max_recovery_file_count() {
        let c = CreateConfig {
            recovery_file_count: Some(31),
            ..valid_config()
        };
        assert!(c.validate().is_ok());
    }
    ...
    #[test]
    fn validate_rejects_first_recovery_block_above_par2_limit() {
        let c = CreateConfig {
            first_recovery_block: 32769,
            ..valid_config()
        };
        assert!(c.validate().is_err());
    }
    ...
    #[test]
    fn validate_accepts_max_first_recovery_block() {
        let c = CreateConfig {
            first_recovery_block: 32768,
            ..valid_config()
        };
        assert!(c.validate().is_ok());
    }
