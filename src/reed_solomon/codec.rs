...
impl ReconstructionEngine {
    ...
    pub fn reconstruct_missing_slices_chunked<W: std::io::Write>(
        ...
        debug!(
            "Available slices (first 20): {:?}",
            &available_slices[..20.min(available_slices.len())]
        );
        debug!("Missing slices: {:?}", global_missing_indices);
        debug!(
            "Base values (first 10): {:?}",
            &self.base_values[..10.min(self.base_values.len())]
        );
        debug!(
            "Base values (last 10): {:?}",
            &self.base_values[self.base_values.len().saturating_sub(10)..]
        );
