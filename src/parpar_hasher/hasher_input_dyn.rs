...
use super::hasher_input_portable::{BlockHash, HasherInput};
...
pub enum HasherInputDyn {
    ...
    Portable(super::hasher_input_portable::HasherInput<super::md5x2_scalar::Scalar>),
}
...
impl HasherInputDyn {
    ...
    /// 2. **BMI1** — fast non-AVX-512 fallback when the CLMUL CRC path is available.
    /// 3. **Scalar** — scalar MD5x2 + CLMUL CRC when `sse4.1` + `pclmulqdq` are available.
    /// 4. **Portable** — scalar MD5x2 + software CRC32 fallback.
    pub fn new() -> Self {
        if Self::should_use_avx512() {
            HasherInputDyn::Avx512(HasherInput::new())
        } else if Self::bmi1_crc_supported() {
            HasherInputDyn::Bmi1(HasherInput::new())
        } else if Self::scalar_crc_supported() {
            HasherInputDyn::Scalar(HasherInput::new())
        } else {
            HasherInputDyn::Portable(super::hasher_input_portable::HasherInput::new())
        }
    }

    fn clmul_crc_supported() -> bool {
        is_x86_feature_detected!("pclmulqdq") && is_x86_feature_detected!("sse4.1")
    }

    fn scalar_crc_supported() -> bool {
        Self::clmul_crc_supported()
    }

    fn sse2_crc_supported() -> bool {
        Self::clmul_crc_supported()
    }

    fn bmi1_crc_supported() -> bool {
        Self::clmul_crc_supported() && is_x86_feature_detected!("bmi1")
    }
    ...
    pub fn new_sse2() -> Self {
        assert!(
            Self::sse2_crc_supported(),
            "SSE2 HasherInput requires sse4.1 + pclmulqdq CRC support"
        );
    ...
    pub fn new_bmi1() -> Self {
        assert!(
            Self::bmi1_crc_supported(),
            "BMI1 HasherInput requires bmi1 + sse4.1 + pclmulqdq support"
        );
        HasherInputDyn::Bmi1(HasherInput::new())
    }
    ...
    pub fn update(&mut self, data: &[u8]) {
        match self {
            ...
            HasherInputDyn::Portable(h) => h.update(data),
        }
    }
    ...
    pub fn get_block(&mut self, zero_pad: u64) -> BlockHash {
        match self {
            ...
            HasherInputDyn::Portable(h) => {
                let block = h.get_block(zero_pad);
                BlockHash {
                    md5: block.md5,
                    crc32: block.crc32,
                }
    ...
    pub fn end(self) -> [u8; 16] {
        match self {
            ...
            HasherInputDyn::Portable(h) => h.end(),
        }
    }
}
...
pub enum HasherInputDyn {
    Scalar(HasherInput<super::md5x2_scalar::Scalar>),
    // TODO: Neon(HasherInput<super::md5x2_neon::Neon>),
    // TODO: ArmCrc(HasherInput<super::crc_armcrc::ArmCrc>),
}
...
impl HasherInputDyn {
    /// Pick the best backend for the current CPU and construct it.
    ///
    /// For now, always uses Scalar (portable, always-available).
    /// Future: Will dispatch to NEON when available.
    pub fn new() -> Self {
        HasherInputDyn::Scalar(HasherInput::new())
    }
...
mod tests {
    ...
    #[test]
    fn default_uses_the_same_backend_family_as_new() {
        assert_eq!(
            HasherInputDyn::default().backend_name(),
            HasherInputDyn::new().backend_name()
        );
    }

    #[cfg(target_arch = "x86_64")]
    #[test]
    fn scalar_and_sse2_backends_match_each_other() {
        let data: Vec<u8> = (0..2048u32).map(|i| (i * 17 + 3) as u8).collect();

        let mut scalar = HasherInputDyn::new_scalar();
        let mut sse2 = HasherInputDyn::new_sse2();

        scalar.update(&data[..1024]);
        sse2.update(&data[..1024]);
        assert_eq!(scalar.get_block(0), sse2.get_block(0));

        scalar.update(&data[1024..]);
        sse2.update(&data[1024..]);
        assert_eq!(scalar.get_block(0), sse2.get_block(0));
        assert_eq!(scalar.end(), sse2.end());
        assert_eq!(HasherInputDyn::new_scalar().backend_name(), "scalar");
        assert_eq!(HasherInputDyn::new_sse2().backend_name(), "sse2");
    }

    #[cfg(target_arch = "x86_64")]
    #[test]
    fn optional_x86_backends_are_guarded_by_runtime_feature_checks() {
        if is_x86_feature_detected!("bmi1") {
            let mut bmi1 = HasherInputDyn::new_bmi1();
            bmi1.update(b"hello world");
            let _ = bmi1.get_block(0);
            assert_eq!(bmi1.end().len(), 16);
            assert_eq!(HasherInputDyn::new_bmi1().backend_name(), "bmi1");
        }

        let maybe_avx512 = HasherInputDyn::try_new_avx512();
        assert_eq!(maybe_avx512.is_some(), HasherInputDyn::avx512_supported());
        if let Some(mut avx512) = maybe_avx512 {
            avx512.update(b"hello world");
            let _ = avx512.get_block(0);
            assert_eq!(avx512.end().len(), 16);
            assert_eq!(
                unsafe { HasherInputDyn::new_avx512() }.backend_name(),
                "avx512"
            );
        }
    }
    ...
    #[cfg(target_arch = "x86_64")]
    #[test]
    fn amd_vector_rotate_probe_returns_a_stable_boolean() {
        let first = is_amd_vec_rot_slow();
        let second = is_amd_vec_rot_slow();
