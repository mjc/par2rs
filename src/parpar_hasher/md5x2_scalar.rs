...
//
// Two-lane scalar (GPR) MD5 block compression — portable across architectures.
//
// On x86_64, uses hand-written inline assembly matching par2cmdline-turbo's
// md5x2-x86-asm.h (AT&T syntax, mirrors the upstream round bodies one-for-one).
//
// On aarch64 and all other targets, uses a pure-Rust implementation ported
// from par2cmdline-turbo's md5-base.h (non-MD5_USE_ASM path with MD5X2 defined).
// rotate_left() lowers to a single `ror` on aarch64 via LLVM.
//
...
//
// Faster tiers (SSE2, AVX-512 on x86_64; NEON on aarch64) layer on top
...
// ============================================================================
// x86_64 asm path (matches par2cmdline-turbo/parpar/hasher/md5x2-x86-asm.h)
// ============================================================================
...
// ============================================================================
// Portable pure-Rust path (aarch64 and other non-x86_64 targets)
// Reference: par2cmdline-turbo/parpar/hasher/md5-base.h (non-MD5_USE_ASM path)
//            with MD5X2 defined — two independent lanes run in lock-step.
// Compiler will lower rotate_left() to a single `ror` on aarch64.
// ============================================================================

#[cfg(not(target_arch = "x86_64"))]
#[inline]
pub unsafe fn process_block_x2_scalar(state: &mut [u32; 8], data1: *const u8, data2: *const u8) {
    // MD5 per-round constants (from RFC 1321 Table T).
    #[rustfmt::skip]
    const K: [u32; 64] = [
        // Round 0 – F
    ...
    // Load 16 little-endian words from each input buffer.
    let mut m1 = [0u32; 16];
    let mut m2 = [0u32; 16];
    for i in 0..16 {
        m1[i] = read32(data1, i);
    ...
    let (mut a1, mut b1, mut c1, mut d1) = (state[0], state[1], state[2], state[3]);
    let (mut a2, mut b2, mut c2, mut d2) = (state[4], state[5], state[6], state[7]);

    // 64 rounds — identical logic for each lane, run in parallel.
    // The compiler interleaves the two instruction streams for free OoO throughput.
    for i in 0..64usize {
        // Message-word index for this round (RFC 1321 §3.4).
        let g = match i {
            0..=15 => i,
            16..=31 => (5 * i + 1) % 16,
            32..=47 => (3 * i + 5) % 16,
        ...
        // Mix function and rotate-add-B for each lane.
        // F = ((c ^ d) & b) ^ d       (equivalent to (b & c) | (!b & d))
        // G = (d & b) | (!d & c)
        // H = d ^ c ^ b
        // I = c ^ (b | !d)
        let (f1, f2) = match i {
            0..=15 => (((c1 ^ d1) & b1) ^ d1, ((c2 ^ d2) & b2) ^ d2),
            16..=31 => ((d1 & b1) | (!d1 & c1), (d2 & b2) | (!d2 & c2)),
            32..=47 => (d1 ^ c1 ^ b1, d2 ^ c2 ^ b2),
            _ => (c1 ^ (b1 | !d1), c2 ^ (b2 | !d2)),
        };

        // _RX: a = (a + f + K[i] + M[g]).rotate_left(R[i]) + b
        //      then rotate state: (a,b,c,d) → (d, a+…, b, c)
        let na1 = a1
            .wrapping_add(f1)
            .wrapping_add(K[i])
            .wrapping_add(m1[g])
