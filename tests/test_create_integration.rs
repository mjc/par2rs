...
/// Helper to create a test file with deterministic non-uniform content
fn create_varied_test_file(path: &Path, size: usize) -> std::io::Result<Vec<u8>> {
...
#[test]
fn test_create_then_corrupt_and_repair_with_par2rs_nonzero_first_recovery_block() {
    let temp = tempdir().unwrap();
    let test_file = temp.path().join("test.dat");
    let par2_file = temp.path().join("test.par2");

    let original_data = create_varied_test_file(&test_file, 4096).unwrap();
    ...
    corrupted_data[128] = !corrupted_data[128];
    corrupted_data[256] = !corrupted_data[256];
    fs::write(&test_file, corrupted_data).unwrap();

    let (_context, result) = par2rs::repair::repair_files(
        par2_file.to_str().unwrap(),
        Box::new(par2rs::repair::SilentReporter),
        &par2rs::verify::VerificationConfig::default(),
    )
    .unwrap();

    assert!(result.is_success(), "repair failed: {result:?}");
...
#[test]
fn basepath_packet_names_are_computed_during_source_scan() {
    let temp = tempdir().unwrap();
    let base = temp.path().join("data");
    let nested = base.join("nested");
    fs::create_dir_all(&nested).unwrap();
    let test_file = nested.join("test.dat");
    let par2_file = temp.path().join("basepath.par2");
...
#[test]
fn zero_byte_source_files_are_skipped_during_create() {
    let temp = tempdir().unwrap();
    let empty_file = temp.path().join("empty.dat");
    let data_file = temp.path().join("data.dat");
    let par2_file = temp.path().join("skip-empty.par2");

    fs::write(&empty_file, []).unwrap();
    create_test_file(&data_file, 1024, 0xBB).unwrap();

    let reporter = Box::new(par2rs::create::ConsoleCreateReporter::new(true));
    let mut context = par2rs::create::CreateContextBuilder::new()
        .output_name(par2_file.to_str().unwrap())
        .source_files(vec![empty_file, data_file])
        .recovery_block_count(1)
        .reporter(reporter)
        .build()
        .unwrap();
