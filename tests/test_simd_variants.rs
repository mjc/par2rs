...
// ARM64 portable_simd tests (compiles to NEON on ARM64)
#[cfg(target_arch = "aarch64")]
mod portable_simd_arm_tests {
    use super::*;
    use par2rs::reed_solomon::simd::process_slice_multiply_add_portable_simd;
    ...
    fn test_portable_simd_correctness() {
        ...
        for size in sizes {
            ...
            // portable_simd implementation (compiles to NEON on ARM64)
            let mut output_simd = vec![0u8; size];
            for (i, byte) in output_simd.iter_mut().enumerate() {
                *byte = (i * 53 + 7) as u8;
            }
            ...
            assert_eq!(
                output_scalar, output_simd,
                "portable_simd mismatch at size {}",
                size
            );
        }
    }
    ...
    fn test_portable_simd_accumulation() {
        ...
        assert_eq!(output, output_scalar, "portable_simd accumulation mismatch");
    }
}
