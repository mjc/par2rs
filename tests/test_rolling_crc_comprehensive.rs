...
fn test_slide_crc_forward_valid() {
    ...
    let current_crc = table
        .compute_crc_at_position(&buffer, 1, block_size, bytes_in_buffer)
        .unwrap();
...
fn test_slide_crc_forward_boundary() {
    ...
    let current_crc = table
        .compute_crc_at_position(&buffer, 32, block_size, bytes_in_buffer)
        .unwrap();
