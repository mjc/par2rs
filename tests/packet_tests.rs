...
use std::io::Read;
use std::path::Path;
use par2rs::{MainPacket, FileDescriptionPacket, InputFileSliceChecksumPacket, RecoverySlicePacket, CreatorPacket};
...
fn test_main_packet_start() {
    ...
    assert_eq!(&main_packet.magic, b"PAR2\0PKT", "Magic field does not match");
    assert!(main_packet.length > 0, "Length field should be greater than 0");
}
...
fn test_file_description_packet_start() {
    ...
    assert_eq!(&file_description.magic, b"PAR2\0PKT", "Magic field does not match");
    assert!(file_description.length > 0, "Length field should be greater than 0");
}
...
fn test_input_file_slice_checksum_packet_start() {
    ...
    assert_eq!(&input_file_slice_checksum.magic, b"PAR2\0PKT", "Magic field does not match");
    assert!(input_file_slice_checksum.length > 0, "Length field should be greater than 0");
}
...
fn test_recovery_slice_packet_start() {
    ...
    file.read_exact(&mut raw_data).expect("Failed to read raw data");
    println!("Raw data: {:?}", raw_data);
    println!("Parsed RecoverySlicePacket: {:?}", recovery_slice);
    assert_eq!(&recovery_slice.magic, b"PAR2\0PKT", "Magic field does not match");
    assert!(recovery_slice.length > 0, "Length field should be greater than 0");
}
...
fn test_creator_packet_start() {
    ...
    assert_eq!(&creator_packet.magic, b"PAR2\0PKT", "Magic field does not match");
    assert!(creator_packet.length > 0, "Length field should be greater than 0");
}
