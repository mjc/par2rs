...
use par2rs::packets::{parse_packets, FileDescriptionPacket};
...
fn test_file_verifier_with_config() {
    ...
    let status =
        verifier.determine_file_status("skip_md5.dat", &md5_16k, &md5_full, content.len() as u64);
...
fn test_determine_file_status_missing() {
    ...
    let status = verifier.determine_file_status("nonexistent.dat", &dummy_hash, &dummy_hash, 1024);
...
fn test_determine_file_status_present() {
    ...
    let status =
        verifier.determine_file_status("perfect.dat", &md5_16k, &md5_full, content.len() as u64);
...
fn test_verify_file_from_description_present() {
    ...
    let file_desc = create_mock_file_desc(file_name, md5_16k, md5_full, content.len() as u64);
...
fn test_verify_file_with_progress_corrupted() {
    ...
    let file_desc = create_mock_file_desc(file_name, wrong_hash, wrong_hash, content.len() as u64);
...
fn test_large_file_16k_optimization() {
    ...
    let status =
        verifier.determine_file_status("large.dat", &md5_16k, &md5_full, content.len() as u64);
...
fn test_small_file_no_16k_optimization() {
    ...
    let status =
        verifier.determine_file_status("small.dat", &md5_16k, &md5_full, content.len() as u64);
