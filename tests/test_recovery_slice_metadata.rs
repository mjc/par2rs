...
fn test_metadata_memory_usage() {
    ...
    assert!(metadata_size < 200, "RecoverySliceMetadata size: {} bytes", metadata_size);
    
    println!("RecoverySliceMetadata size: {} bytes", metadata_size);
    println!("RecoverySlicePacket base size: {} bytes (excluding recovery_data Vec)", 
...
fn test_parse_from_reader() {
    ...
    let metadata = RecoverySliceMetadata::parse_from_reader(
        &mut cursor,
        PathBuf::from("/test/file.par2")
    ).unwrap();
