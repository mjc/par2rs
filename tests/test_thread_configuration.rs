...
fn test_thread_configuration_values() {
    ...
    let configs = [
        ...
        (2, true),  // Two threads, parallel
        (4, true),  // Four threads, parallel
        (8, true),  // Eight threads, parallel
        (0, false), // Auto-detect threads, sequential
        (4, false), // Four threads, sequential (threads ignored)
    ];
    ...
    for (threads, parallel) in configs.iter() {
        ...
        // Basic validation
        if !config.parallel {
            // In sequential mode, thread count should not matter for correctness
            // but we still store it for potential future use
        }

        if config.threads == 1 && config.parallel {
            // Single thread parallel is effectively sequential
            // but still uses parallel iterators (just with 1 thread)
        }
    }
}
...
fn test_thread_pool_configuration() {
    ...
    for config in test_configs.iter() {
        ...
        if config.threads > 0 {
            ...
            match result {
                Ok(_) => println!("Successfully configured {} threads", config.threads),
...
fn test_parallel_vs_sequential_flag() {
    ...
    // Both should have same thread count but different parallel flag
...
fn test_config_edge_cases() {
    ...
    // Sequential mode ignores thread count for processing but stores it
